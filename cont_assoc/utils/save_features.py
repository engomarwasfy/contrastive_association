...
def save_features(x, raw_feat, sem_pred, ins_pred, save_preds):
    for i in range(len(x['pcd_fname'])):
        ...
        if save_preds:
            ...
            ins =  ins_pred[i]
            valid = ins != 0
            seq_path = f'data/validation_predictions/sequences/{seq}/'
            max_pt = 30
        else:
            valid = x['pt_valid'][i]
            sem = x['pt_labs'][i]
            ins =  x['pt_ins_labels'][i]
            seq_path = f'data/instance_features/sequences/{seq}/'
            max_pt = 10
        ...
        filename = f'{seq_path}scans/{fname}'
        if not os.path.exists(seq_path):
            os.makedirs(f'{seq_path}scans/')
            orig_seq_path = x['pcd_fname'][i][:-19]
            #copy txt files
            copyfile(f'{orig_seq_path}poses.txt', f'{seq_path}poses.txt')
            copyfile(f'{orig_seq_path}calib.txt', f'{seq_path}calib.txt')
            #create empty file
            f = open(f'{seq_path}empty.txt', 'w')
            f.close()
        if not save_preds and not _ids:
            with open(f'{seq_path}empty.txt', 'a') as f:
                f.write(fname+'\n')
            continue
        if save_preds:
